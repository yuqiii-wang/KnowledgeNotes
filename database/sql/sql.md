...
It is a common use scenario such that, for example, given a table `PRODUCT`, there is `PRODUCT.ID` that increments by `1` everytime it receives an `INSERT` commit.

Implementations on MySQL, PostgreSQL and Oracle

|Feature|MySQL|PostgreSQL|Oracle|
|-|-|-|-|
|Native Auto-Increment Support|Yes (`AUTO_INCREMENT`)|Yes (`SERIAL`/`IDENTITY`)|No|
|Sequence Required|Implicit|Implicit (for `SERIAL`)|Explicit|
|Trigger Required|No|No|Yes|

### MySQL by `AUTO_INCREMENT`
...
### PostgreSQL by `SERIAL`

Use the `SERIAL` pseudo-type (or `GENERATED BY DEFAULT`/`ALWAYS AS IDENTITY` for newer PostgreSQL versions).
...
### Oracle by `SEQ` and `TRIGGER`

Oracle does not have an `AUTO_INCREMENT` keyword. Instead, need to manually create a sequence and optionally use a trigger to automate it.

...
For a query token $\mathbf{q}_m\in\mathbb{R}^{1\times D}$ positioned at $m$ and a key token $\mathbf{k}_n\in\mathbb{R}^{1\times D}$ at $n$, and assume their respective dimensions correlate to the two token distance $|n-m|$ (corresponding frequency denoted as $\frac{1}{|n-m|}$, also distance is termed *wavelength*),
...
$$
\begin{align*}
    \mathbf{freqDimTrans}(\Delta)=\bigg[ & w_{1}\Pi(\frac{1}{1}\Delta), w_{2}\Pi(\frac{1}{2}\Delta), ..., w_{50}\Pi(\frac{1}{50}\Delta), ..., w_{300}\Pi(\frac{1}{300}\Delta), ..., w_{1000}\Pi(\frac{1}{1000}\Delta), \bigg]
\end{align*}
$$
...
Apply $\mathbf{freqDimTrans}(\Delta)$ to "dogs", "cats", and "pets", only certain frequencies are fired up thereby having learned distance info.
...
Define a score to be maximized when query $\mathbf{q}_m$ is positionally "close" to key $\mathbf{k}_n$.
...
$$
\max \text{score}(\mathbf{q}_m, \mathbf{k}_n) =
(R_{n} \mathbf{q}_1)^{\top} (R_{m} \mathbf{k}_1) =
\mathbf{q}_1^{\top} R_{n}^{\top}  R_{m} \mathbf{k}_1 =
\mathbf{q}_1^{\top} R_{n-m} \mathbf{k}_1
$$

Now use and $\theta_i \in (10^{-4}, 1]$ such that $\theta_i=10000^{-\frac{2i}{\mathbf{D}}}$ to assign discrete values to $R_{n-m}$.

Let $D$ represent the dimension num of $\mathbf{v} \in \mathbb{R}^{1 \times D}$.
Let $R(\theta)$ be a rotation matrix for a vector $\mathbf{v}$, there is
...
Let $\mathbf{v} \in \mathbb{R}^{1 \times D}$, group dimensions by pairs: $(v_1, v_2), (v_3, v_4), ..., (v_{D-1}, v_{D})$
...
For $\mathbf{v} \in \mathbb{R}^{256}$, group by pairing
...
Given the assumption (as an example) that query vector $\mathbf{q}_m$ sees key vector $\mathbf{k}_n$
at $n=m+\Delta$ (relative position distance is $\Delta$), compute the score by rotation (replace $\mathbf{v}$ with $\mathbf{q}_m$ and $\mathbf{k}_n$).
...
Large $\theta_i$ leads to significant oscillation of $\langle \mathbf{q}_m, \mathbf{k}_n \rangle$ given distance changes $|n-m| \theta_i$.

* Low Frequency

Small $\theta_i$ leads to insensitivity to distance changes $|n-m| \theta_i$ hence $\langle \mathbf{q}_m, \mathbf{k}_n \rangle$ oscillation is small.

##### Frequency Study In Long Distance

When two tokens are very distant $|n-m|=\Delta\rightarrow \infty$, the score $\langle \mathbf{q}_m, \mathbf{k}_n \rangle$ has multiple mappings (for $R(\theta)$ is a periodic function) hence the attention score cannot determine which query token be associated to which key token.
...
See in $\langle \mathbf{q}_m, \mathbf{k}_n \rangle=\sum_{i=0}^{127} \big(\alpha_{\cos}\cos(|n-m|\theta_i)+\alpha_{\sin}\sin(|n-m|\theta_i) \big)$,
as token distance $|n-m|$ grows from closely adjacent to faraway,
when tokens are positioned closely the inner product $\langle \mathbf{q}_m, \mathbf{k}_n \rangle$ is dominated by cosine $\cos(|n-m|\theta_i)\approx 1$,
while sine is trivial $\sin(|n-m|\theta_i)\approx 0$.

As a result, in the beginning the inner product $\langle \mathbf{q}_m, \mathbf{k}_n \rangle$ sees monotonic decrease.
...
and since the start of non-decrease oscillation happens, the attention score by $\mathbf{q}_m^{\top}\mathbf{k}_n$ no longer holds discriminative semantics.
...
The rotated vector is $\mathbf{v}\ne\mathbf{0}\in\mathbb{R}^{2}$.
...
$$
R(-\theta_a)R(\theta_a)\mathbf{v}=R(-\theta_a)R(\theta_b)\mathbf{v}
$$
...
Now reconsider the original hypothesis if it can be assumed that two different angles $\theta_{a}$ and $\theta_{b}$ can map to the same rotation $R(\theta_a)=R(\theta_b)$ on the same $\mathbf{v}$.
The derived expression $\mathbf{v}=R(-\theta_a+\theta_b)\mathbf{v}$ led result $\theta_a=\theta_b\in[0, 2\pi)$ rejected the hypothesis.
...
This leads to loss of resolution that resonated two embedding $\mathbf{v}_m^{\top}\mathbf{v}_n$ can see multiple mappings.

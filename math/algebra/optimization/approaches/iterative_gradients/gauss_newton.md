...
In data fitting, where the goal is to find the parameters $\mathbf{\beta}$ to a known  model $\mathbf{f}(\mathbf{x}, \mathbf{\beta})$ that best fits observation data $(x_i, y_i)$, there is

$$
r_i = y_i - f(x_i, \mathbf{\beta})
$$ 

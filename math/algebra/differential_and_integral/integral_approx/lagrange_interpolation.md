...
If the interpolation points $x_0, x_1, ..., x_n$ are distinct, then the process of finding a polynomial that passes through the points $(x_i, y_i)$ for $i=0,1,2,...,n$ is equivalent to solving a system of linear equations $A\mathbf{x}=\mathbf{b}$ that has a unique solution.

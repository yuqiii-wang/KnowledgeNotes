...
let $\mathbf{x}-\mathbf{\mu}=[x_1-\mu_1, x_2-\mu_2, ..., x_n-\mu_n]^{\top}$, and define the covariance matrix as
...
$$
\big(\frac{x_1-\mu_1}{\sigma_1}\big)^2+\big(\frac{x_2-\mu_2}{\sigma_2}\big)^2+...+\big(\frac{x_n-\mu_n}{\sigma_n}\big)^2 = (\mathbf{x}-\mathbf{\mu})^{\top}\Sigma^{-1}(\mathbf{x}-\mathbf{\mu})
$$

so,

$$
p(\mathbf{x}) = \frac{1}{\sqrt{(2\pi)^n|\Sigma|}}\exp\bigg({-\frac{1}{2}(\mathbf{x}-\mathbf{\mu})^{\top}\Sigma^{-1}(\mathbf{x}-\mathbf{\mu})}\bigg)
$$

The above expression can be noted as $\mathbf{x}\sim\mathcal{N}(\mathbf{\mu}, \Sigma)$.
...
Assume prior $f(\mathbf{x})\sim\mathcal{N}(\mathbf{\mu}_f, \Sigma_{ff})$ follows the Gaussian process, and there are some observation $(\mathbf{x}^*, \mathbf{y}^*)$ that are considered following the Gaussian process as well, i.e.,
...
Here uses RBF as kernel function $\kappa(\mathbf{x}, \mathbf{x}')$.

In the figure below, the Gaussian process starts from the prior distribution with $\mu_f=\mathbf{0}$ and $\Sigma_{ff}=\sigma_f^2I$.
...
where $\Sigma_{yy}=\kappa(\mathbf{x}^*, \mathbf{x}^*)$.

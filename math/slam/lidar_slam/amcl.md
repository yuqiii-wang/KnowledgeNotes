...
Generic Problem
For a 2-d localization problem, to estimate the state of the robot at the current time-step $k$. Given knowledge about the initial state $x_0$ and all measurements $z^k = \{ Z_k, i = 1:k \}$, to work out $\mathbf{x}=[x,y,\theta]^T$
...
$$
p(\mathbf{x_k}|Z^k) = \frac{p(\mathbf{z_k}|\mathbf{x_k})p(\mathbf{x_k}|Z^{k-1})}{p(\mathbf{z}_k|Z^{k-1})}
$$

where, $\mathbf{z}_k$ is the sensor measurement data. $p(\mathbf{z}_k|Z^{k-1})$ is same as $p(\mathbf{z}_k)$ since each sensor measurement is independently from its previous. 

Measurement model is given in terms of a likelihood $p ( \mathbf{z}_k | \mathbf{x}_k )$ , describing the likelihood that a robot is at $\mathbf{x}_k$ given observation $\mathbf{z}_k$.

## Monte Carlo Localization

In sampling-based methods one represents the density $p ( \mathbf{x}_k | Z^k )$ by a set of $N$ random samples or particles $S_k = \{ s^i_k ; i = 1,2, ..., N \}$ drawn from it.
...
Observation $\mathbf{z}_k$ says the robot is nearer to a top-right corner landmark (about $0.5$ meter)

* D

Combine the information from step B and C, $p(\mathbf{z_k}|\mathbf{x_k})p(\mathbf{x_k}|Z^{k-1})$ gives the estimations of the sampled states $\mathbf{x}_k$

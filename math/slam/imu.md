...
Typically, it meaasures rotation $\bold{R}$, translation $\bold{p}$, angular velocity $\bold{\omega}$, linear velocity $\bold{v}$ and acceleration $\bold{a}$ in a 3D space.

## IMU Calibration

IMU calibrated output $[W_x \quad W_y \quad W_z]^\top$ can be formulated as
$$
\begin{bmatrix}
    W_x \\
    W_y \\
...
Zero drift is zero offset stability (the standard deviation of direct measurement), which represents the degree of dispersion of gyroscope output around its zero offset mean value when the input angular rate is zero. 
...
There are noises in IMU: measurement noises between two keyframe $\bold{\eta}_{ij}$ and zero offset/biases to angular and linear movements $\bold{b}_{\omega}$ and $\bold{b}_{a}$. During integration, such noises (assumed Gaussian noises) can be approximated by linear increments, hence avoided repeated noise computation.

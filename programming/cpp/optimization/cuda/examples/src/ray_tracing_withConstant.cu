...
__constant__ Sphere sphGpu[SphereNums];


__global__ void kernel(uchar* imgGpuData)
{
...
int main()
{
    ...
    err = cudaMemcpyToSymbol(sphGpu, sphCpu, sizeof(Sphere)*SphereNums);
    if (err != 0) {
        std::cout << "sphGpu cudaMemcpyToSymbol err" << std::endl;
        std::cout << cudaGetErrorString ( err ) << std::endl;
    }

    dim3 grids(ImageWidth/16, ImageHeight/16);
    dim3 threads(16, 16);
    kernel<<<grids, threads>>>(imgGpuData);
